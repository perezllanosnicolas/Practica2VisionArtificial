"""
MSER-based Highway Panel Detector (OCR Module).

Pipeline:
  1. Maximally Stable Extremal Regions (MSER) to detect high-contrast areas.
  2. Geometric filtering of bounding boxes (size, aspect ratio).
  3. Bounding box expansion to capture the full blue background.
  4. HSV validation scoring against a strict ideal mask with boundary penalties.
"""
...
class PanelDetectorMSER:
    """
    Detects information panels using MSER to identify areas with high contrast,
    followed by geometric and strict topological color validation.
    """
    ...
    def __init__(self, min_area: int = 500, hue_min: int = 90, hue_max: int = 130, 
        ...
        # Blue HSV range (Strictly matching the [hue, sat, 50] rule)
        ...
        # Ideal validation mask (Calculated once to save CPU cycles)
    ...
    def _create_ideal_mask(self, h: int, w: int) -> np.ndarray:
        """Creates an ideal synthetic mask with a -2 penalty border."""
        mask = np.ones((h, w), dtype=np.float32)
        # 4-pixel border to heavily penalize over-segmentation (sky, cars, etc.)
        mask[0:4, :] = -2.0
        mask[-4:, :] = -2.0
        mask[:, 0:4] = -2.0
        mask[:, -4:] = -2.0
